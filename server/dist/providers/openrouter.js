...
function getNextOpenRouterKey() {
    if (config_1.config.openRouterKeys.length === 0)
        return undefined;
    const key = config_1.config.openRouterKeys[currentKeyIndex];
    currentKeyIndex = (currentKeyIndex + 1) % config_1.config.openRouterKeys.length;
    return key;
}
function getAllKeys() {
    return [...config_1.config.openRouterKeys];
}
function generateRequestId() {
    return `chatcmpl-${Date.now().toString(36)}_${Math.random().toString(36).substring(2, 9)}`;
}
function generateRotatingIp() {
    const octet = () => Math.floor(Math.random() * 256);
    return `${octet()}.${octet()}.${octet()}.${octet()}`;
}
async function createChatCompletion(request, requestId) {
    const modelId = (0, models_1.resolveModelId)(request.model);
    ...
    // Build request body
    const maxTokensSafetyCap = config_1.config.maxTokensSafetyCap;
    const defaultMaxTokens = config_1.config.defaultMaxTokens;
    ...
    if (totalInputChars > MAX_PROVIDER_CHARS) {
        ...
        if (systemMsgIdx !== -1) {
            ...
            const otherMessages = body.messages.filter((_, i) => i !== systemMsgIdx);
            ...
            for (let i = otherMessages.length - 1; i >= 0; i--) {
                ...
                if (usedChars + msgJson.length > allowedChars)
                    break;
    ...
    try {
        const response = await fetch(config_1.config.openRouterUrl, {
            method: 'POST',
            headers,
            body: JSON.stringify(body),
            signal: AbortSignal.timeout(config_1.config.providerTimeoutMs),
        });
        if (!response.ok) {
            ...
            if ((response.status === 429 || response.status === 401 || response.status === 403) && config_1.config.openRouterKeys.length > 1) {
                const fallbackKeys = getAllKeys().filter(k => k !== apiKey);
                for (const fallbackKey of fallbackKeys) {
                    console.log(`[${requestId}] Trying fallback OpenRouter key...`);
                    try {
                        const fallbackResponse = await fetch(config_1.config.openRouterUrl, {
                            method: 'POST',
