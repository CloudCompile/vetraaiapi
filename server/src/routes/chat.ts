...
import { createChatCompletion } from '../providers/openrouter';
import { config } from '../config';
import { CHAT_MODELS, getModelById, modelExists, resolveModelId } from '../models';
import { rateLimitMiddleware, checkDailyLimit, getDailyLimitInfo } from '../middleware/rateLimiter';
import { apiKeyAuth, AuthenticatedRequest } from '../middleware/auth';
import { validateChatCompletion, validateMessages } from '../middleware/validation';
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!req.isSystemRequest && !checkDailyLimit(req as any, dailyLimit)) {
      const dailyInfo = getDailyLimitInfo(req as any, dailyLimit);
      res.set({
        ...getCorsHeaders(),
    ...
    // Validate model
    const resolvedModelId = resolveModelId(body.model);
    if (!modelExists(body.model)) {
    ...
    // Call provider
    const providerResult = await createChatCompletion(body, requestId);
              ...
              try {
                ...
                if (parsed.choices && parsed.choices[0]?.message?.content_blocks) {
                  ...
                  if (textContent && !parsed.choices[0].delta?.content) {
                    if (!parsed.choices[0].delta) parsed.choices[0].delta = {};
