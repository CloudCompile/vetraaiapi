...
function getRateLimitForRequest(req: Request): { limit: number; windowMs: number } {
  // Check if system request
  const authHeader = req.headers.authorization;
  if (authHeader?.startsWith('Bearer ')) {
    const key = authHeader.slice(7).trim();
    if (config.systemApiKey && key === config.systemApiKey) {
...
export function checkDailyLimit(req: Request, maxDaily: number = 1000): boolean {
  const identifier = getClientIdentifier(req);
...
export function getDailyLimitInfo(req: Request, maxDaily: number = 1000): { remaining: number; resetAt: number; limit: number } {
  const identifier = getClientIdentifier(req);
  const today = new Date().toISOString().split('T')[0];
